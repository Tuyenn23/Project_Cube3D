    ...
    public static T Instance
    {
        get
        {
            ...
            if (objects.Length > 0)
            {
                ...
                if (objects.Length > 1)
                {
                    //DebugCustom.LogWarning("There is more than one instance of Singleton of TypeShop \"" + TypeShop + "\". Keeping the first. Destroying the others.");
            ...
            if (attribute == null)
            {
                //DebugCustom.LogError(TypeShop + "class does not have SingletonAttribute ! Please add SingletonAttribute for " + TypeShop);
                return null;
            }
            if (string.IsNullOrEmpty(attribute.PathInstance))
            {
                //DebugCustom.LogError("Cannot find prefab of "+ TypeShop);
                return null;
            }
            ...
            if (prefab == null)
            {
                //DebugCustom.LogError("Cannot find prefab of " + TypeShop + "! Put prefab of" + TypeShop + " into Resources folder");
                return null;
            }
